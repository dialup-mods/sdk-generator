...
enum EFunctionFlags : uint64_t {
  ...
  FUNC_Public       = 0x00020000, // Function is accessible in all classes (if
                                  // overridden, parameters much remain unchanged).
  FUNC_Private      = 0x00040000, // Function is accessible only in the class it is defined in
                                  // (cannot be overriden, but function name may be reused in
  ...
  FUNC_FuncOverrideMatch = ( FUNC_Exec     | FUNC_Final  | FUNC_Latent | FUNC_PreOperator |
                             FUNC_Iterator | FUNC_Static | FUNC_Public | FUNC_Protected   |
                             FUNC_Const ),
...
enum EPropertyFlags : uint64_t {
  ...
  CPF_Component          = 0x0000000000080000, // Property contains component references.
  CPF_AlwaysInit         = 0x0000000000100000, // Property should never be exported as NoInit(@todo -this doesn't need to
                                               // be a property flag... only used during make).
  ...
  CPF_RepRetry           = 0x0000002000000000, // Property replication of this property if it fails to be fully sent
                                               // (e.g. object references not yet available to serialize over the network).
  ...
  CPF_CrossLevelPassive  = 0x0000100000000000, // Property can point across levels, and will be
                                               // serialized properly, but assumes it's target
                                               // exists in-game (non-editor)
  CPF_CrossLevelActive   = 0x0000200000000000, // Property can point across levels, and will be
...
enum EObjectFlags : uint64_t {
    ...
    RF_MisalignedObject       = 0x0000000000002000, // Object's size no longer matches the size of its C++ class (only used during make,
                                                    // for native classes whose properties have changed).
    ...
    RF_Load        = ( RF_ContextFlags       | RF_LoadContextFlags   | RF_Public    | RF_Standalone     |
                       RF_Native             | RF_Obsolete           | RF_Protected | RF_Transactional  |
                       RF_PerObjectLocalized | RF_ClassDefaultObject | RF_ArchetypeObject | RF_HasStack |
    ...
    RF_DefaultOrArchetypeFlags = RF_ClassDefaultObject | RF_ArchetypeObject,
...
enum EPackageFlags : uint32_t {
  ...
  PKG_StoreFullyCompressed        = 0x04000000, // Package is serialized normally, and then fully compressed after
                                                // (must be decompressed before LoadPackage is called)
...
enum EClassFlags : uint32_t {
  ...
  CLASS_HasInstancedProps  = 0x00200000, // class contains object properties which are marked "instanced" (or edit inline export).
  CLASS_NeedsDefProps      = 0x00400000, // Class needs its default properties imported.
  ...
  CLASS_Inherit          = ( CLASS_Transient          | CLASS_Config             | CLASS_Localized        |
                             ...
                             CLASS_HasInstancedProps  | CLASS_HasCrossLevelRefs  | CLASS_SafeReplace      ),
